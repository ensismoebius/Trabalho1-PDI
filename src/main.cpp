...
void exercicio06() {
	...
	cv::Mat opening = cv::Mat();
	cv::Mat coinsBg = cv::Mat();
	cv::Mat coinsFg = cv::Mat();
	cv::Mat distTrans = cv::Mat();
	cv::Mat unknown = cv::Mat();
	cv::Mat markers = cv::Mat();
	...
	for (int i = 0; i < markers.rows; i++) {
		...
		uchar *ptrM = markers.ptr<uchar>(i);
		uchar *ptrG = gray.ptr<uchar>(i);

		for (int j = 0; j < markers.cols; j++) {
			ptrM[j] = ptrM[j] + 1;
			if (ptrG[j] == 255) {
				markers.at<uint>(i, j) = 0;
			}
		}
	}
	...
	cv::watershed(src, gray);

	// draw barriers
	for (int i = 0; i < markers.rows; i++) {
		for (int j = 0; j < markers.cols; j++) {
			if (markers.ptr<int>(i, j)[0] == -1) {
				src.ptr<uchar>(i, j)[0] = 255; // R
				src.ptr<uchar>(i, j)[1] = 255; // G
				src.ptr<uchar>(i, j)[2] = 255; // B
			}
		}
	}

	cv::imshow("opening", opening);
	cv::imshow("distTrans", distTrans);
	cv::imshow("src", src);
	cv::waitKey();
}
